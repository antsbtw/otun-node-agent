...
// HotReloadClient 调用 WP-A fork sing-box 暴露的本地热更端点（标准节点 WP-C）。
//
// 契约（WP-A 已定，权威，与 realm WP-1 同源）：
...
//     统一 {"ok":false,"error":"..."}。
//
// 该端点同时换 VLESS 认证 map + 刷 v2ray_api 计费集 → 一次调用即可，加用户不断现有连接。
// 连接被拒（sing-box 没起 / 老二进制 v1.10.7 无端点）→ 返回 error，调用方据此降级回 reload。
...
// hotReloadUser 是请求体里的单个用户（仅 uuid，对齐 WP-A 契约）。
...
// 这是全量 set 语义：传完整列表，sing-box 据此替换 VLESS 认证集 + v2ray_api 计费集。
// 不断现有连接。失败返回 error，调用方据此降级回 reload。
